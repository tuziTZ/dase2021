...
void Execve(const char *filename, char *const argv[], char *const environ[])//p525
{
...
void eval(char *cmdline);//p525
int builtin_cmd(char **argv);//p525
...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        ...
        if (!bg) {
            ...
            printf("[%d] (%d) %s",pid2jid(pid), pid, cmdline);//test04
        }
    }
    return;
}
...
void do_bgfg(char **argv) 
{
    ...
    else {            // Checks if the second argument is valid
