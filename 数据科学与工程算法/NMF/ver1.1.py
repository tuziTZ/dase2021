...
def NMF(V, K=10, times=50):
    row, col = V.shape
    W = np.around(np.array(np.random.rand(row, K)), 5)
    H = np.around(np.array(np.random.rand(col, K)), 5)
    rmse_list=[]
    mae_list=[]
...
if __name__ == "__main__":
    query_num = 6
    x=[]
    with open("./ml-10M100K/ratings.dat", 'r') as file:
    ...
    for i in range(len(old_list)):
        if old_list[i] > max and new_list[i] > 0:
            max = old_list[i]
            index = i
