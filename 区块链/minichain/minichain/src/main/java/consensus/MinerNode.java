...
public class MinerNode extends Thread {
    ...
    public BlockBody getBlockBody(Transaction[] transactions) {
        ...
        //todo
        //计算出merkleRootHash
        String[] transactionHashes = new String[transactions.length];
        for (int i = 0; i < transactions.length; i++) {
            transactionHashes[i] = SHA256Util.sha256Digest(transactions[i].toString());
        }
        while (transactionHashes.length > 1) {
            int newSize = (transactionHashes.length + 1) / 2;
            String[] newHashes = new String[newSize];

            for (int i = 0; i < newSize; i++) {
                int leftIndex = i * 2;
                int rightIndex = Math.min(leftIndex + 1, transactionHashes.length - 1);

                String combinedHash = SHA256Util.sha256Digest(transactionHashes[leftIndex]+transactionHashes[rightIndex]);
                newHashes[i] = combinedHash;
            }
        ...
        //创建实例
        return new BlockBody(transactionHashes[0],transactions);
    }
    ...
    public Block getBlock(BlockBody blockBody) {
        ...
        String preBlockHash=SHA256Util.sha256Digest(preblock.toString());
