 ...
 */
  ...
  unsigned int min_size=2147483647;
  ...
  do {
    ...
    while (hp != NIL_HOLE && hp->h_base < swap_base) {
      if (hp->h_len >= clicks&&hp->h_len < min_size) {
        new_hp=hp;
    ...
    if(new_hp!=NIL_HOLE){
      ...
      old_base = new_hp->h_base;	/* remember where it started */
      new_hp->h_base += clicks;	/* bite a piece off */
      new_hp->h_len -= clicks;	/* ditto */
